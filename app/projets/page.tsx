...
const projects = [
  ...
  },
  {
    ...
    icon: Warehouse,
    categories: ["Data Engineering", "Software Engineering", "Cloud"] as Category[],
    role: "Projet personnel — proto hybride Azure + poste local",
    featured: true,
    description:
      "Seconde horloge de MicroRecouv : détecter un impayé entre deux DAGs du matin. CDC en lecture seule sur PostgreSQL, bus Kafka, lake Delta Bronze / Silver / Gold. Le calcul (règles 30 / 60 / 90 / 180 j, API) reste sur le PC ; Azure porte le disque (ADLS Gen2) et le bus (Event Hubs). Ce n'est pas un entrepôt Fabric ni le rapport COBAC — c'est une file d'alertes opérationnelles, isolées par imf_id.",
    highlights: [
      "CDC Postgres (bac à sable ou Stage) → Event Hubs Kafka (SASL_SSL) ; profil local MinIO + Redpanda inchangé",
      "Tables Delta sur ADLS Gen2 (abfss://) : Bronze brut, Silver qualité, Gold alertes persistées",
      "Moteur d'impayés + FastAPI : signal recouvrement en quasi temps réel, pas le PAR réglementaire du batch dbt",
    ],
    hardSkills: [
      "Python",
      "CDC",
    ...
    softSkills: [
      ...
      "Honnêteté de périmètre (proto, pas prod)",
